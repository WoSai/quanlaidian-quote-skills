...
3. PDF 文件直接发送到对话
- 调用报价脚本生成文件后，从输出中提取 PDF 链接（`result["files"]["pdf"]["url"]`），用 `curl -L -o /tmp/<品牌名>-全来店-报价单.pdf "<PDF URL>"` 下载到本地。
- 使用 `message` 工具（`action=send`、`channel=feishu`、`filePath`、`caption`）把 PDF 文件直接发送到当前对话；`caption` 用 `📄 <品牌名>-全来店-报价单.pdf`。
- 发送成功后，在配置摘要下方加一句提示：`📎 报价单 PDF 已直接发送到本对话。`
- **不阻塞流程**：若 PDF 下载失败、发送失败、或当前环境（非飞书）不支持附件发送，静默跳过本步，仍按第 2 步返回 3 个下载链接作为 fallback，不报错、不中断。
- 注意：`curl -L` 必须带 `-L` 跟进短链 302 重定向（不加 -L 会把 302 响应体存成文件）；文件命名固定用 `品牌名-全来店-报价单.pdf`。
