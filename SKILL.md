...
`scripts/quote.py` 输出的是**原始材料**（文本段落），Agent 须按以下模板将其组装为最终回复。飞书平台 Markdown 环境下，使用**二级标题 + 表格 + emoji** 的排版结构，避免使用长分隔线（如 `━━━`、`---`），防止飞书渲染为灰色背景块。
...
| 项目 | 报价 | 成本 | 利润 | 利润率 |
|------|:----:|:----:|:----:|:------:|
| {item} | ¥{price} | ¥{cost} | ¥{profit} | {rate}% |
```

#### 模板填充规则

- **判定来源**：行业关键字自动判定时写 `已根据关键词「{词}」自动判定`；用户主动选择时写 `用户选择`；缺省不写
- **自动修正 ⚠️ 块**：触发互斥哨兵清空/配送加工自动切套餐/自动追加门店点位时，抽成以下格式插入在配置摘要和费用明细之间：
  ```
  > ⚠️ **配置已自动调整**
  > - **{原因A}**：{说明}
  > - **{原因B}**：{说明}
  ```
  各条目的 `{原因}` 加粗关键词，如「按**无门店增值模块**处理，其他勾选已忽略」「为承接配送/生产加工自动切换为**全能版**」
...
- **利润评估**：使用 `---` 短分隔线与费用明细隔离（飞书渲染 `---` 为细线，不会出现灰色背景块）
...
> 注意：`scripts/quote.py` 输出的是**原始材料**。Agent 须把这些原始段落按上节『报价输出推荐排版模板』组装进最终的对话回复中（分隔线 + 表格 + emoji），不能直接贴原始输出也不能跳过某些段落。
